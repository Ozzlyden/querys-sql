        ...
        FROM
            (
                ...
                WHERE
                    ...
                    AND a.dh_documento BETWEEN TO_DATE($PgIgesdfDtInicial$) AND TO_DATE($PgIgesdfDtFim$) + 0.99999
                    AND a.tp_status = 'FECHADO'
                    AND a.cd_atendimento IN (
                        SELECT cd_atendimento
                        FROM atendime
                        WHERE cd_multi_empresa in ($PgEmpresasIgesDF$)
                    )
            ) pdc
            LEFT JOIN (
                SELECT
        ...
        FROM
            (
                ...
                FROM
                    dbamv.atendime
                WHERE
                    NVL(dt_alta_medica, dt_alta) BETWEEN TO_DATE($PgIgesdfDtInicial$) AND TO_DATE($PgIgesdfDtFim$) + 0.99999
                ...
                FROM
                    dbamv.atendime
                WHERE
                    NVL(dt_alta_medica, dt_alta) BETWEEN TO_DATE($PgIgesdfDtInicial$) AND TO_DATE($PgIgesdfDtInicial$) + 0.99999
                    AND cd_multi_empresa in ($PgEmpresasIgesDF$)
                    AND cd_mot_alt IN (
                        SELECT cd_mot_alt
                        FROM dbamv.mot_alt

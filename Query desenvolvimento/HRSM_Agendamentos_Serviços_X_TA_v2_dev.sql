...
WHERE 
    a.hr_agenda BETWEEN TO_DATE('01/07/2024') AND TO_DATE('31/07/2024') + 0.99999
...
SELECT DISTINCT
    ...
    END AS tipo_medico
/*select distinct 
    a.cd_atendimento, a.cd_multi_empresa, a.cd_paciente, a.dt_atendimento, a.tp_atendimento, a.cd_prestador, c.nm_prestador, a.cd_especialid, a.cd_procedimento*/
FROM
    dbamv.atendime a
...
WHERE
    a.dt_atendimento BETWEEN TO_DATE('01/07/2024') AND TO_DATE('31/07/2024') + 0.99999
    AND a.tp_atendimento = 'E'
    AND a.cd_multi_empresa IN (2)
    --AND a.dt_alta IS NULL
    --AND a.cd_procedimento = 0301010072
    --AND a.cd_especialid = 38
    --AND c.nm_prestador like ('%ANDREA%')
GROUP BY
    a.cd_especialid,
    b.ds_especialid,
    c.cd_tip_presta
